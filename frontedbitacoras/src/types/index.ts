...
export type UserSignUpT = Pick<AdminT, 'username' | 'password'>;

export type UserLocalStorageT = Pick<
  UserT,
  'full_name' | 'role_id' | 'cooperative_id'
>;
...
export type editFrequencyT = Partial<FrequencyT> & {
  id: string;
  license_plate?: string;
  driver_dni?: string;
};
...
export type TypeCameraT = {
  id_camra: number;
  nombre: string;
  id_nave_per: number;
};

export type TypeComercianteT = {
  id_comerciante: number;
  nombre: string;
  cupo: string;
  cedula: string;
  ciu_comerciante: string;
  id_producto_per: string;
  cupo_semana: string;
  activo: string;
};

export type TypeConsignaT = {
  id_consigna: number;
  fecha: Date;
  dirigido: string;
  id_usuario_per: string;
  consigna_especial: string;
  estado: string;
  omv: string;
  fecha_verificacion: Date;
  hora_verificacion: Date;
};

export type TypeIngresoT = {
  id_ingreso: number;
  id_puesto_per: number;
...
export type TypePuestoT = {
  id_puesto: number;
  nombre: string;
  cupo: number;
  id_nave_per: number;
  id_comerciante_per: number;
  cupo_semana: number;
};

export type TypeInterfaceT = {
  id_rol: number;
  nombre: string;
};

export type TypeUsuarioT = {
  id_usuario: number;
  cedula: string;
  usuario: string;
  contrasenia: string;
